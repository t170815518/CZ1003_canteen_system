...
def change_time():  # author: Tang Yuting
    ...
    def confirm_to_change_time():
        ...
        try:  # exception handling
            ...
            wkd = datetime.strptime(tmp_list[0]+' '+tmp_list[1]+' '+tmp_list[2], '%Y %m %d').strftime('%A')  # display weekday
            # [2000, 10, 23, 00, 30, 1]
        except:
...
def back_to_current(): # author: Tang Yuting
    ...
    stall_update()
    menu_initialization()
    wkd = datetime.strptime(str(TIME[0]) + ' ' + str(TIME[1]) + ' ' + str(TIME[2]), '%Y %m %d').strftime('%A')
    time_str.set('{:4d}-{:2d}-{:2d} {:02d}:{:02d} {}'.format(TIME[0], TIME[1], TIME[2], TIME[3], TIME[4], wkd))
...
def view():  # author: Tang Yuting
    ...
    if mode == 1 or m_mode == 1:
        info_heading.set('Complete Version of Menu')
        menus = get_menu(st.get(st.curselection()), TIME, mode=1)
    else:
        info_heading.set('Menu')
        menus = get_menu(st.get(st.curselection()), TIME, mode=0)
